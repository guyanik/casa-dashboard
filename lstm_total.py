...
df_pred = pd.DataFrame(hstack((X, inv_y, inv_yhat)))
df_pred.columns = ['QUANTITY', 'VOLUME', 'WEIGHT', 'PRICE', 'SECONDSSPENT', 'PREDICTION']
df_pred['DATE'] = date
df_pred.to_csv('data/lstm_total_prediction.csv', index=False)
